from model import MyInceptionV3, Classifier
...
def get_args():
    ...
    parser.add_argument('--input', '-t', type=str, default=None,
                        help='Load test data for evaluation')
    parser.add_argument('--output', '-o', type=str, default=None,
                        help='Output directory path for predition masks')
    parser.add_argument('--model', '-m', type=str,
                        default=None, help='Model path to load')
...
def inference(datapath, outpath, modelpath, batch_size=8):
    ...
    df = pd.DataFrame(
        {'filename': inference_set.filenames, 'label': result})
